...
def plot_img(msk, area, original, out, fsaverage):
     for side in ['right', 'left']:
         ...
         mask_img = nilearn.image.new_img_like(maps, work_msk)
         #### save masks
         mask_img.to_filename(os.path.join(out, '{}_{}_mask.nii'.format(side, area)))
         ...
         nilearn.plotting.plot_glass_brain(
                                       atl_img, 
                                       output_file=os.path.join(out, '{}_{}_glass.jpg'.format(side, area),)
                                       )
         ### Right
         if side == 'right':
